...
#define adc_temp_enable()			WRITE_CBUS_REG_BITS(SAR_ADC_REG0, 1, 15, 1)
...
#define set_ctrl02(in) WRITE_CBUS_REG_BITS(SAR_ADC_REG3, in, 23, 3)
...
void saradc_temp_sw(unsigned mode)
{
    set_tempsen(mode);
}

void saradc_ctrl02_sw(unsigned in)
{
    set_ctrl02(in);
}

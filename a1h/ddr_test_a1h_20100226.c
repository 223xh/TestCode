...
//    ddr_write(test_buff_c, rand_num);
    ...
    {   
        ...
        ddr_write(test_buff, TEST_DATA_A, j);
        ddr_read(test_buff, TEST_DATA_A, j);
        
        memset(test_buff, 0, TEST_SIZE);
        ddr_write(test_buff, TEST_DATA_B, j);
        ddr_read(test_buff, TEST_DATA_B, j);
        
        memset(test_buff, 0, TEST_SIZE);
        srand(AVTimeGet());
        rand_num = (unsigned)rand();
        ddr_write(test_buff, rand_num, j);
        ddr_read(test_buff, rand_num, j);
#endif
    }

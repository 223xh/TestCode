...
static const unsigned int  signal_set[SIGNAL_SET_MAX][3]=
{
	...
	{
		VIDEO_SIGNAL_TYPE_PROGEESSIVE_G,	//Analog RGB for VGA.
		VIDEO_SIGNAL_TYPE_PROGEESSIVE_B,
...
void  change_vdac_setting(unsigned int  vdec_setting,vmode_t  mode)
{
	...
	DEBUG_VGA();
	printk("VENC_VDAC_DACSEL0:0x%x\n",READ_MPEG_REG(VENC_VDAC_DACSEL0));
	printk("VENC_VDAC_DACSEL1:0x%x\n",READ_MPEG_REG(VENC_VDAC_DACSEL1));
	printk("VENC_VDAC_DACSEL2:0x%x\n",READ_MPEG_REG(VENC_VDAC_DACSEL2));
}
...
static void tvoutc_set_pinmux(tvmode_t mode)
{
	...
	{	        
		...
		WRITE_MPEG_REG(PERIPHS_PIN_MUX_0, READ_MPEG_REG(PERIPHS_PIN_MUX_0) & ~(0x3 << 15));  //clear TCON_STH1, TCON_STV1
		WRITE_MPEG_REG(PERIPHS_PIN_MUX_11, READ_MPEG_REG(PERIPHS_PIN_MUX_11) |  (0x3 <<  0));  //set HDMI_HS, HDMI_VS		
	}
	else
	{
		WRITE_MPEG_REG(PERIPHS_PIN_MUX_11, READ_MPEG_REG(PERIPHS_PIN_MUX_11) & ~(0x3 <<  0));		
	}	
}
